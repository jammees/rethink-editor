...
local Themes = require(script.Parent.Themes).GetTheme()

local Fusion = require(library.Fusion)
local Children = Fusion.Children
local New = Fusion.New

local function RenderObjectManipulation()
	-- StG = Snap to Grid
	local function RenderStG()
		local handlersStG = {
			H_checkbox.new({
				Title = "Snap to Grid",
				InitialValue = false,
		...
		H_unifier.unifiedXSize(handlersStG)

		return table.unpack(handlersStG)
	end

	return H_toolbarCategory({
		Separator = "right",
...
local function RenderDebugButtons()
			...
			for class, props in exportedData do
				...
				in props do
					...
					elseif typeof(objProperty) == "UDim2" then
						...
						serialized = `UDim2.new({udim2.X.Scale}, {udim2.X.Offset}, {udim2.Y.Scale}, {udim2.Y.Offset})`
